...
def convert(dumpsdir, outputdir, single_file, do_parallel, nthreads, pad):
  ...
  if single_file:
    iharm_filename = os.path.join(outputdir, os.path.basename(dumpsdir).replace(".phdf5", ".h5"))
    ...
    iharm_files  = [os.path.join(outputdir, os.path.basename(f).replace(".phdf5", ".h5")) for f in kharma_files]
