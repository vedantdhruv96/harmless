...
diagnostic_dict = {
    ...
    "betaInv": lambda dump, G=None, indices=None, **kwargs: grid.inv_scalar(
        calc_plasma_beta(dump, G)
    ),
    ...
    "ucov": lambda dump, G=None, indices=None, **kwargs: grid.lower_vec(
        calc_ucon(dump, G), G
    ),
    "bcon": lambda dump, G=None, indices=None, **kwargs: calc_bcon(dump, G),
    "bcov": lambda dump, G=None, indices=None, **kwargs: grid.lower_vec(
        calc_bcon(dump, G), G
    ),
    "bsq": lambda dump, G=None, indices=None, **kwargs: calc_bsq(dump, G),
    "lorentzFactor": lambda dump, G=None, indices=None, **kwargs: calc_lorentz_factor(
        dump, G
    ),
    # Stress-energy tensor (contravariant T^{mu nu})
    "Tcon": lambda dump, G=None, indices=None, **kwargs: calc_Tcon(dump, G, indices),
    "TconFl": lambda dump, G=None, indices=None, **kwargs: calc_TconFl(
        dump, G, indices
    ),
    "TconEM": lambda dump, G=None, indices=None, **kwargs: calc_TconEM(
        dump, G, indices
    ),
    # Stress-energy tensor (covariant T_{mu nu})
    "Tcov": lambda dump, G=None, indices=None, **kwargs: calc_Tcov(dump, G, indices),
    "TcovFl": lambda dump, G=None, indices=None, **kwargs: calc_TcovFl(
        dump, G, indices
    ),
    "TcovEM": lambda dump, G=None, indices=None, **kwargs: calc_TcovEM(
        dump, G, indices
    ),
    # Mixed stress-energy tensor (T^mu_nu)
    "Tmixed": lambda dump, G=None, indices=None, **kwargs: calc_Tmixed(
        dump, G, indices
    ),
    "TmixedFl": lambda dump, G=None, indices=None, **kwargs: calc_TmixedFl(
        dump, G, indices
    ),
    "TmixedEM": lambda dump, G=None, indices=None, **kwargs: calc_TmixedEM(
        dump, G, indices
    ),
    "MaxwellStress": lambda dump, G=None, indices=None, **kwargs: calc_Maxwell_stress(
        dump, G, indices
    ),
    "TPAKE": lambda dump, G=None, indices=None, **kwargs: calc_TPAKE(dump, G, indices),
    "TEN": lambda dump, G=None, indices=None, **kwargs: calc_T_thermal(
        dump, G, indices
    ),
...
def calc_q(dump):
    ...
    if dump.ho_conduction:
        cs2 = calc_cs2(dump)
        Theta = (dump.gam - 1.0) * dump.u / dump.rho
...
def calc_ucon(dump, G):
    ...
    qsq = (
        G.gcov[..., 1, 1] * dump.u1**2
        + G.gcov[..., 2, 2] * dump.u2**2
        + G.gcov[..., 3, 3] * dump.u3**2
        + 2.0
        * (
            G.gcov[..., 1, 2] * dump.u1 * dump.u2
...
def _build_TconEM(dump, G):
    ...
    T = (
        bsq[..., np.newaxis, np.newaxis]
        * ucon[..., :, np.newaxis]
        * ucon[..., np.newaxis, :]
        + (bsq / 2.0)[..., np.newaxis, np.newaxis] * G.gcon
        - bcon[..., :, np.newaxis] * bcon[..., np.newaxis, :]
    )
    return T
